...
func (r *userRepository) IsEmailExists(email string) (bool, error) {
	...
	if err := r.db.Raw(`
		SELECT COUNT(1)
		FROM User
		WHERE Email = ?`, email).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) IsMobileNumberExists(mobile string) (bool, error) {
	...
	if err := r.db.Raw(`
		SELECT COUNT(1)
		FROM User
		WHERE Mobile = ?`, mobile).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) IsEmailExistsExceptID(id uint, email string) (bool, error) {
	...
	if err := r.db.Raw(`
		SELECT COUNT(1)
		FROM User
		WHERE ID <> ? AND Email = ?`, id, email).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) IsMobileNumberExistsExceptID(id uint, mobile string) (bool, error) {
	...
	if err := r.db.Raw(`
		SELECT COUNT(1)
		FROM User
		WHERE ID <> ? AND Mobile = ?`, id, mobile).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) IsUserCodeExists(code string) (bool, error) {
	...
	if err := r.db.Raw(`
		SELECT COUNT(1)
		FROM User
		WHERE Code = ?`, code).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) IsUserCodeExistsExceptID(id uint, code string) (bool, error) {
	...
	if err := r.db.Raw(`
		SELECT COUNT(1)
		FROM User
		WHERE ID <> ? AND Code = ?`, id, code).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) FetchUsers(filters *request.CommonRequest) (*utils.PaginationResponse, error) {
	var data []response.FetchUsers
	...
	baseQuery.WriteString(`
		...
		[User].UpdatedAt, [User].IsActive, COUNT(1) OVER (PARTITION BY 1) AS userCount, 
	...
	if len(filters.Search) > 0 {
		baseQuery.WriteString(` AND ([User].Email LIKE ? OR [User].FirstName LIKE ? 
			OR [User].LastName LIKE ?)`)
		queryParams = append(queryParams, search, search, search)
	}

	baseQuery.WriteString(` ORDER BY [User].CreatedAt DESC`)
...
func (r *userRepository) UpdateUser(userID uint, req *request.UpdateUser) error {
	return r.db.Exec(`
				UPDATE [User]
				SET UpdatedAt = ?, FirstName = ?, LastName = ?, Code = ?, Email = ?, 
				Mobile = ?, RoleID = ?
				WHERE ID = ?`,
		time.Now(), req.FirstName, req.LastName, req.Code,
		req.Email, req.Mobile, req.RoleID, userID).
		Error
}

func (r *userRepository) FetchUserStatus() ([]response.FetchUserStatus, error) {
	var data []response.FetchUserStatus

	if err := r.db.Raw(`
		SELECT ID, [Name]
		FROM UserStatus`).Scan(&data).Error; err != nil {
		return nil, err
	}
...
func (r *userRepository) IsUnmappedLeadUserIncludeUserID(userID uint) (bool, error) {
	...
	if err := r.db.Raw(`
		WITH tempTable AS(
			...
			FROM User usr
			LEFT JOIN DepartmentMember dm ON dm.UserID = usr.ID AND dm.IsActive = 1
			WHERE usr.roleID IN ? AND usr.IsActive = 1 AND ( dm.UserID = ? OR dm.UserID IS NULL)) 
			
			SELECT COUNT(1)
			FROM tempTable
			WHERE tempTable.userID = ?;`,
		[]interface{}{constant.DepartmentLead, constant.HR}, userID, userID).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) FetchUnmappedLeadUsers() ([]response.FetchUnmappedLeadUsers, error) {
	var data []response.FetchUnmappedLeadUsers
...
func (r *userRepository) FetchUnmappedLeadUserIncludeUserID(req *request.FetchUnmappedLeadUserIncludeUserID) ([]response.FetchUnmappedLeadUsers, error) {
	var data []response.FetchUnmappedLeadUsers
...
func (r *userRepository) FetchUnmappedEmployeeUsers() ([]response.FetchUnmappedEmployeeUsers, error) {
	var data []response.FetchUnmappedEmployeeUsers
...
func (r *userRepository) IsEmployeeUserExists(id uint) (bool, error) {
	var count int64
	...
	if err := r.db.Raw(`
		...
		FROM User usr
		LEFT JOIN DepartmentMember dm ON dm.UserID = usr.ID AND dm.isActive = 1
		WHERE usr.ID = ? AND usr.roleID = ? AND usr.IsActive = 1 AND dm.UserID IS NOT NULL`,
		id, constant.Employee).Scan(&count).Error; err != nil {
		return false, err
	}
...
func (r *userRepository) FetchUserDetails(req *request.FetchUserDetails) (*response.FetchUserDetails, error) {
	...
	if err := r.db.Raw(`
		...
		FROM [User] usr
		LEFT JOIN UserKyc uk ON usr.ID = uk.UserID AND usr.IsActive = 1
		WHERE usr.ID = ?`,
...
func (r *userRepository) FetchUnmappedHRUserIncludeUserID(req *request.FetchUnmappedLeadUserIncludeUserID) ([]response.FetchUnmappedLeadUsers, error) {
	var data []response.FetchUnmappedLeadUsers
...
func (r *userRepository) FetchUnmappedHRUsers() ([]response.FetchUnmappedEmployeeUsers, error) {
	var data []response.FetchUnmappedEmployeeUsers
