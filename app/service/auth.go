...
func (s *authService) Login(req *request.Login) (*response.FetchUserByEmail, error) {
	...
	if user.RoleID == uint(constant.Employee) && user.DepartmentID == nil {
		return nil, fmt.Errorf("You are not assigned to any department. Kindly contact HR")
	}
