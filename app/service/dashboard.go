...
func (s *dashboardService) FetchDashboardCounts(roleID, departmentMemberID, departmentID uint) (interface{}, error) {
	...
	case int(constant.Admin), int(constant.Manager), int(constant.HR):
		{
			...
			leaveCount, err := s.leaveRepository.GetLeaveCount()
			if err != nil {
				return nil, err
			}

			approvedLeaveCount, err := s.leaveRepository.GetApprovedLeaveCount()
			if err != nil {
				return nil, err
			}

			permissionCount, err := s.permissionRepository.GetPermissionCount()
			if err != nil {
				return nil, err
			}

			approvedPermissionCount, err := s.permissionRepository.GetApprovedPermissionCount()
			if err != nil {
				return nil, err
			}
	...
	case int(constant.DepartmentLead):
		{
			...
			leaveCount, err := s.leaveRepository.GetLeaveCountByDepartment(departmentID)
			if err != nil {
				return nil, err
			}

			approvedLeaveCount, err := s.leaveRepository.GetApprovedLeaveCountByDepartment(departmentID)
			if err != nil {
				return nil, err
			}

			permissionCount, err := s.permissionRepository.GetPermissionCountByDepartment(departmentID)
			if err != nil {
				return nil, err
			}

			approvedPermissionCount, err := s.permissionRepository.GetApprovedPermissionCountByDepartment(departmentID)
			if err != nil {
				return nil, err
			}
	...
	case int(constant.Employee):
		{
			...
			leaveCount, err := s.leaveRepository.GetLeaveCountByUser(departmentMemberID)
			if err != nil {
				return nil, err
			}

			approvedLeaveCount, err := s.leaveRepository.GetApprovedLeaveCountByUser(departmentMemberID)
			if err != nil {
				return nil, err
			}
			...
			approvedPermissionCount, err := s.permissionRepository.GetApprovedPermissionCountByUser(departmentMemberID)
			if err != nil {
				return nil, err
			}
