...
func (h *DepartmentHandler) FetchDepartments(c *gin.Context) {
	...
	data, err := h.departmentService.FectchDepartments(&filters)
...
func (h *DepartmentHandler) MappUsersToDepartment(c *gin.Context) {
	...
	api_response.Success(c, "Employees mapped to the department successfully", nil)
}
...
func (h *DepartmentHandler) UnMapEmployee(c *gin.Context) {
	var req request.UnMapEmployee
	...
	if err := h.departmentService.UnMapEmployee(&req); err != nil {
		api_response.InternalServerError(c, err.Error())
		return
	}

	api_response.Success(c, "Employee unmapped successfully", nil)
}
